...
export class TonContract {
  ...
  async callKeys({ functionName, input, keys }: { functionName: string; input?: any, keys?:any }) {
    if (process.env.DEBUG) {
      console.log("functionName", functionName);
      console.log("input", input);
    }
  ...
  async callInternal({ functionName, input = {}, value = "", address = "" }: { functionName: string; input?: {}, value?: string, address?: string }) {
    const account = await this.client.net.query_collection({
      collection: "accounts",
      filter: { id: { eq: this.address } },
      result: "boc data",
    ...
    const message = await this.client.tvm.run_tvm({
      message: (
        await this.client.abi.encode_internal_message({
          abi: { type: "Contract", value: this.tonPackage.abi },
          call_set: {
            function_name: functionName,
            input,
